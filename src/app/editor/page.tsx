...
function EditorContent() {
  ...
  return (
    <div className="flex flex-col h-screen bg-white">
        ...
        <div className="flex items-center gap-0 px-1 py-0.5 border-t border-[#f0f0f0] overflow-x-auto">
          {[
            ...
            <button
              key={i}
              onClick={btn.action}
              className="p-2 text-[#d1d5db] hover:text-[#1a1a1a] rounded"
            >
              {btn.icon}
            </button>
          ))}
      ...
      {/* Bottom action bar */}
      <div className="flex items-center gap-3 px-4 py-2 border-t border-[#f0f0f0]">
        <button
        ...
        {isNote && hasContent && (
          <button
            ...
            className="text-[11px] flex items-center gap-1 text-[#4F46E5] font-medium"
          >
        ...
        {hasContent && (
          <button
            ...
            className="text-[11px] flex items-center gap-1 text-[#6b7280] font-mono"
          >
        ...
        {(isPrompt || isTemplate) && hasContent && (
          <button
            ...
            className="text-[11px] flex items-center gap-1.5 bg-[#1a1a1a] text-white px-3 py-1.5 rounded-full font-medium"
          >
      ...
      <div className="bg-white w-full max-w-lg rounded-t-2xl p-6 space-y-3" onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={() => onPromote("prompt")} className="w-full flex items-center gap-3 p-4 rounded-xl border border-[#f0f0f0] hover:border-[#4F46E5]/30 hover:bg-[#EEF2FF]/20">
          <div className="w-9 h-9 rounded-lg bg-[#EEF2FF] flex items-center justify-center">
            <WandSparkles className="w-4 h-4 text-[#4F46E5]" />
          </div>
          <div className="text-left flex-1">
            <p className="font-medium text-sm">Prompt</p>
            <p className="text-[10px] text-[#9ca3af] font-mono">Save as AI instruction</p>
          </div>
        ...
        <button onClick={() => onPromote("template")} className="w-full flex items-center gap-3 p-4 rounded-xl border border-[#f0f0f0] hover:border-[#9ca3af]/30">
          ...
          <div className="text-left flex-1">
            ...
            <p className="text-[10px] text-[#9ca3af] font-mono">Reusable with variables</p>
          </div>
          <ChevronRight className="w-4 h-4 text-[#d1d5db]" />
        </button>
      ...
      <div className="bg-white w-full max-w-lg rounded-t-2xl p-6 space-y-2 max-h-[80vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="space-y-1">
          {AI_APPS.map((app) => (
            <button
              ...
              className="w-full flex items-center gap-3 p-3 rounded-xl border border-[#f0f0f0] hover:border-[#d1d5db]"
            >
              <span className="text-lg w-8 h-8 flex items-center justify-center rounded-lg bg-[#fafafa]">
                {app.icon}
              </span>
              <div className="text-left flex-1">
                <p className="font-medium text-sm">{app.name}</p>
                <p className="text-[10px] text-[#9ca3af] font-mono">Copy & open</p>
              </div>
              {copiedId === app.id ? (
        ...
        <div className="border-t border-[#f0f0f0] pt-2">
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-[#fafafa]"
          >
            <span className="w-8 h-8 flex items-center justify-center bg-[#f5f5f5] rounded-lg">
              <Copy className="w-4 h-4 text-[#6b7280]" />
            </span>
            <div className="text-left flex-1">
              <p className="font-medium text-sm">Copy only</p>
              <p className="text-[10px] text-[#9ca3af] font-mono">To clipboard</p>
            </div>
...
function AIReviewSheet({ bodyMd, onApply, onClose }: {
  ...
  const gradeColor: Record<string, string> = {
    A: "text-[#1a1a1a]", B: "text-[#404040]", C: "text-[#4F46E5]", D: "text-red-500"
  };
  const axisLabels: Record<string, string> = {
    clarity: "Clarity", specificity: "Specificity", structure: "Structure", context: "Context", constraints: "Constraints"
  };
      ...
      <div className="bg-white w-full max-w-lg rounded-t-2xl p-6 max-h-[85vh] overflow-auto" onClick={(e) => e.stopPropagation()}>
        ...
        {state === "idle" && (
          <div className="space-y-4">
            <p className="text-xs text-[#9ca3af] text-center font-mono">Evaluate your prompt on 5 axes</p>
            ...
            <div className="bg-[#fafafa] p-3 rounded-lg border border-[#f0f0f0]">
              <p className="text-[10px] text-[#9ca3af] mb-1 font-mono uppercase tracking-wider">Target</p>
              <p className="text-sm text-[#404040] line-clamp-4 leading-relaxed">{bodyMd}</p>
            </div>
        ...
        {state === "loading" && (
          <div className="text-center py-16">
            <div className="animate-spin w-5 h-5 border-2 border-[#1a1a1a] border-t-transparent rounded-full mx-auto mb-4" />
            <p className="text-[#9ca3af] text-xs font-mono">Reviewing...</p>
          </div>
        )}
        ...
        {state === "done" && (
          <div className="space-y-4">
            <div className="text-center py-4">
              <p className="text-[10px] text-[#9ca3af] mb-1 font-mono uppercase tracking-wider">Overall</p>
              <p className={`text-4xl font-bold ${gradeColor[overall] || ""}`}>{overall}</p>
            </div>

            <div className="space-y-2">
              {Object.entries(scores).map(([key, val]) => (
                <div key={key} className="flex items-center gap-2 text-xs">
                  <span className="w-20 text-[#9ca3af] font-mono text-[11px]">{axisLabels[key] || key}</span>
                  <span className={`font-bold w-5 ${gradeColor[val.grade] || ""}`}>{val.grade}</span>
                  <span className="text-[#9ca3af] text-[11px] flex-1">{val.feedback}</span>
                </div>
              ))}
            </div>

            <div className="bg-[#EEF2FF]/30 border border-[#EEF2FF] p-4 rounded-xl">
              <p className="font-medium text-xs mb-2 text-[#4F46E5] font-mono uppercase tracking-wider">Improved version</p>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 py-2.5 border border-[#f0f0f0] text-[#6b7280] font-medium rounded-xl text-xs hover:bg-[#fafafa]"
              >
                Copy
              </button>
              <button
            ...
            <div className="flex items-center justify-center gap-4 text-[11px] text-[#d1d5db]">
              ...
              <button className="p-2 hover:text-[#1a1a1a] rounded-full"><ThumbsUp className="w-3.5 h-3.5" /></button>
              <button className="p-2 hover:text-[#1a1a1a] rounded-full"><ThumbsDown className="w-3.5 h-3.5" /></button>
            </div>
          </div>
        )}
        ...
        {state === "error" && (
          <div className="text-center py-12 space-y-3">
            <p className="text-[#9ca3af] text-xs font-mono">Review failed</p>

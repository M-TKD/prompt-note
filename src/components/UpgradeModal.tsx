...
import { X, Sparkles, History, FolderOpen, Headphones, Loader2 } from "lucide-react";
...
export function UpgradeModal({ onClose }: UpgradeModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <button onClick={onClose} className="absolute top-4 right-4 text-[#d1d5db] hover:text-[#6b7280] dark:hover:text-white">
          <X className="w-4 h-4" />
        </button>
        ...
        <div className="text-center space-y-1 pt-2">
          ...
          <p className="text-xs text-[#9ca3af] font-mono">すべての機能を制限なく利用</p>
        </div>

        <div className="space-y-2.5">
          {FEATURES.map((f, i) => (
            <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-[#fafafa] dark:bg-[#222] border border-[#f0f0f0] dark:border-[#333]">
              <div className="w-8 h-8 rounded-lg bg-[#EEF2FF] dark:bg-[#4F46E5]/20 flex items-center justify-center text-[#4F46E5]">
                {f.icon}
              </div>
              <span className="text-sm font-medium text-[#404040] dark:text-[#e5e7eb]">{f.label}</span>
            </div>
          ))}
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="block w-full py-3 bg-[#4F46E5] hover:bg-[#4338CA] text-white font-medium rounded-xl text-sm text-center transition-colors disabled:opacity-60"
        >
          {loading ? (
        ...
        <button onClick={onClose} className="w-full text-center text-[11px] text-[#d1d5db] hover:text-[#9ca3af] py-1">
          閉じる
        </button>
      </div>
    </div>
